...
- **本地优先，网络可感知**：输入先落库，AI 整理异步执行并以流式阶段反馈连接、分析、生成和保存状态；网络波动时由后台自动重试，提醒与洞察始终围绕本地数据运行。
...
- **AI 意图分流**：一次调用区分「笔记 / 待办」。主整理通过 SSE 流式接收，详情页显示准备、连接、分析、生成、保存与自动重试阶段；待办自动生成截止时间与本地提醒；笔记自动整理出标题、分类、类型、情绪、标签、摘要，并提炼其中可执行的待办
...
- **三协议自由接入**：OpenAI Chat Completions / OpenAI Responses / Anthropic Messages，Base URL 任意填——官方 API、DeepSeek、各类中转、本地 Ollama 均可；Responses 支持在用户开启模型思考且主动允许后显示提供商返回的简短推理摘要，摘要不写入数据库
...
- **AI 层适配器模式**：`LlmAdapter` 统一接口收敛三协议差异为 `LlmRequest / LlmResult`，主整理同时支持受控 SSE 事件流；完整 JSON 输出才进入三级兜底解析与落库
- **离线可靠**：输入先落库（PENDING_AI），AI 流水线走 WorkManager（联网约束 + 指数退避）；流式连接中断、截断或 JSON 不完整均不会写入半截结果，重试耗尽后才显示失败入口
...
./gradlew :app:testDebugUnitTest    # JVM 单元测试（121 个：三协议与流式 SSE 契约、心跳/断流/取消、摘要隐私、弱网重试、JSON 兜底、URL/HTML 安全策略、图表校验、合并输入、关联算法与 Release 解析）
...
| 模型思考 | 可开关，支持低 / 中 / 高强度；配置贯穿 OpenAI Chat、OpenAI Responses 和 Anthropic Messages。Responses 在启用思考后可额外选择显示提供商返回的推理摘要，默认关闭且不落库 | — |
...
- JVM 单元测试：`121 tests completed, 0 failed`
- 已通过 `:app:testDebugUnitTest`、`:app:assembleDebug`、`:app:assembleRelease`、`:app:lintDebug`；Release 版本为 `0.6.0`（versionCode `13`）
- 流式适配器验证覆盖 OpenAI Chat、Responses 与 Anthropic Messages 的 SSE 心跳、终止事件、断流、取消、回退和多模态请求；未使用真实 API Key 或端点

## 历史验证（2026-08-31）

- JVM 单元测试：`80 tests completed, 0 failed`
- 已通过 `:app:compileDebugAndroidTestKotlin`、`:app:assembleDebug`；历史 Release 版本为 `0.5.0`（versionCode `7`）
