...
### 改进
- 主 Prompt 新增明确的 `is_inspiration` 判定标准：用户自己产生的新想法/点子/观点/顿悟/建议雏形标记为灵感；引用摘录、客观事实、资料整理、总结复盘、待研究问题标记为非灵感；犹豫时默认 false，宁缺毋滥
- 「自动」模式不再依赖模型隐式语义，判定结果更稳定可控


### 新增
- 笔记合并时可勾选「合并完成后删除原笔记」：删除原笔记正文、附件和关联，其提炼的待办保留
- 速记页新增灵感标记三选一（自动判断 / 标记灵感 / 非灵感），用户可在输入时显式控制是否打上灵感标签，AI 结果按用户意图强制写入 is_inspiration

### 修复
- 修复首页「已删除笔记」等操作提示一直停留在列表顶部的问题：提示 3 秒后自动消失


### 修复
- 修复首页“全部”筛选错误：灵感笔记不再因 `inspirationOnly = false` 被排除；全量数量与实际列表保持一致
- 增加回归测试，确保普通全部视图不会传递灵感过滤条件


### 重建知识网络
- 重建任务现在显示排队、运行中、处理进度、当前阶段、完成数量和失败原因
- 设置页监听 WorkManager 真实状态，重建完成后显示处理笔记数和关联数
- 关联发现未启用时不再创建无效任务，直接提示先启用
- 双向关联按实际关系数量统计，避免重复计数
